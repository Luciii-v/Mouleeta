...
/**
 * OtpVerificationModal — Unified OTP modal for MOULEETA V.2
 *
 * Routing:
 *   type === "email"  → Resend email delivery via /api/otp/send + /api/otp/verify
 *   type === "phone"  → Firebase Phone Authentication (SMS via Google's carrier network)
 *
 * Both paths create a real NextAuth session on success.
 */

import React, { useState, useEffect, useRef, lazy, Suspense } from "react";
import { signIn } from "next-auth/react";

// Lazy-load Firebase phone flow to avoid bundling Firebase into all pages
const PhoneOtpFlow = lazy(() => import("@/components/PhoneOtpFlow"));
...
export default function OtpVerificationModal({
  ...
  const [devOtp, setDevOtp] = useState(""); // Only populated in dev mode (no provider)
  const [countdown, setCountdown] = useState(30);
  const inputRefs = useRef([]);

  // ── Email OTP: send via Resend (/api/otp/send) ────────────────────────────
  ...
  useEffect(() => {
    let timer;
    if (isOpen && type === "email" && countdown > 0) {
  ...
  // ── Email OTP: verify via /api/otp/verify ─────────────────────────────────
          ...
          <>
            ...
            <div className="text-center mb-6">
              ...
              <h3 className="text-sm font-semibold uppercase tracking-[0.15em] text-stone-900">
                Privé Email Verification
              </h3>
              <p className="text-xs text-stone-500 mt-1.5 leading-relaxed">
                {sending ? "Dispatching your verification code via Resend…" : (
                  <>
                    Code sent to <span className="font-medium text-stone-800">{target}</span>. Enter below to verify.
                  </>
                )}
              </p>
            </div>

            {/* Dev-mode banner — only visible in development when no provider is configured */}
            {devOtp && (
              <div className="mb-6 bg-stone-900 text-white p-3 rounded-sm flex items-center justify-between text-xs tracking-wider border border-stone-800 shadow-inner">
                <div>
                  <span className="text-stone-400 text-[10px] uppercase block mb-0.5">Dev Mode — Resend not configured</span>
            ...
            <div className="text-center">
              {countdown > 0 ? (
                ...
                >
                  {sending ? "Sending…" : "Resend via Resend"}
                </button>
              )}
            </div>
          </>
