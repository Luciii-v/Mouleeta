"use client";
import React from 'react';
import Image from 'next/image';
import Link from 'next/link';
import { signIn } from 'next-auth/react';

export default function RegisterPage() {
  return (
    <main className="min-h-screen flex bg-[#F9F8F6]">
      
      {/* LEFT SIDE: Cinematic Editorial Image */}
      <div className="hidden lg:block lg:w-1/2 relative">
        <Image 
          src="/linen-shirt.png" 
          alt="Mouleeta Menswear" 
          fill 
          className="object-cover object-center grayscale-[10%]"
          priority
        />
        <div className="absolute inset-0 bg-black/10" />
      </div>

      {/* RIGHT SIDE: Minimalist Form */}
      <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 sm:px-16 md:px-24 xl:px-32 py-24">
        
        <div className="max-w-md w-full mx-auto animate-fadeIn">
          <h1 className="text-3xl font-light tracking-[0.2em] text-stone-900 uppercase mb-2">
            Create Account
          </h1>
          <p className="text-sm text-stone-500 font-light tracking-widest mb-10 uppercase leading-relaxed">
            Join Mouleeta securely with Google.
          </p>

          <button 
            type="button" 
            onClick={() => signIn('google', { callbackUrl: '/account' })}
            className="w-full flex items-center justify-center gap-3 bg-stone-900 text-white text-xs tracking-[0.2em] uppercase py-4 hover:bg-black hover:shadow-lg transition-all duration-300 cursor-pointer"
          >
            <svg className="w-4 h-4" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
            </svg>
            Continue with Google
          </button>

          {/* Login Link */}
          <div className="mt-12 text-center border-t border-stone-200 pt-8">
            <p className="text-[11px] tracking-widest uppercase text-stone-500 mb-4">
              Already have an account?
            </p>
            <Link 
              href="/account/login" 
              className="inline-block border border-stone-900 text-stone-900 px-8 py-3 text-xs tracking-[0.2em] uppercase hover:bg-stone-900 hover:text-white transition-colors duration-300 w-full"
            >
              Sign In
            </Link>
          </div>

        </div>
      </div>
    </main>
  );
}
